...
describe("routes", () => {
  ...
  test("serves existing-project ship guidance and versioned source", async () => {
    ...
    const source = await app.request("/ship/v43.ts");
    ...
    expect(sourceBody).toContain('const CURRENT_SOURCE = "https://shibumistack.dev/ship/v43.ts"');
    ...
    expect(installerRedirect.headers.get("location")).toBe("/ship/install-v41.ts");
    const installer = await app.request("/ship/install-v41.ts");
    ...
    expect(installerBody).toContain("ship/v43.ts");
