...
describe("routes", () => {
  ...
  test("serves existing-project ship guidance and versioned source", async () => {
    ...
    const source = await app.request("/ship/v45.ts");
    ...
    expect(sourceBody).toContain('const CURRENT_SOURCE = "https://shibumistack.dev/ship/v45.ts"');
    ...
    expect(installerRedirect.headers.get("location")).toBe("/ship/install-v43.ts");
    const installer = await app.request("/ship/install-v43.ts");
    ...
    expect(installerBody).toContain("ship/v45.ts");
    ...
    expect(bootstrapRedirect.headers.get("location")).toBe("/ship/bootstrap-v29.sh");
    const bootstrap = await app.request("/ship/bootstrap-v29.sh");
    expect(bootstrap.status).toBe(200);
    expect(bootstrap.headers.get("cache-control")).toContain("immutable");
    const bootstrapBody = await bootstrap.text();
    expect(bootstrapBody).toContain("ship/install-v39.ts");
