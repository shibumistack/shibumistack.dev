...
describe("routes", () => {
  ...
  test("serves existing-project ship guidance and versioned source", async () => {
    ...
    expect(body).toContain('fetch("/ship/v19.ts")');
    ...
    const source = await app.request("/ship/v19.ts");
    ...
    expect(installerRedirect.headers.get("location")).toBe("/ship/install-v17.ts");
    const installer = await app.request("/ship/install-v17.ts");
    ...
    expect(installerBody).toContain("Installer changes were rolled back");
    ...
    expect(bootstrapRedirect.headers.get("location")).toBe("/ship/bootstrap-v10.sh");
    const bootstrap = await app.request("/ship/bootstrap-v10.sh");
    expect(bootstrap.status).toBe(200);
    expect(bootstrap.headers.get("cache-control")).toContain("immutable");
    expect(await bootstrap.text()).toContain("ship/install-v17.ts");
