...
describe("routes", () => {
  ...
  test("serves existing-project ship guidance and versioned source", async () => {
    ...
    expect(body).toContain('fetch("/ship/v7.ts")');
    expect(body).toContain("data-copy-code");
    expect(body).not.toContain('href="/ship/v7.ts"');
    ...
    const source = await app.request("/ship/v7.ts");
    ...
    expect(installerRedirect.headers.get("location")).toBe("/ship/install-v5.ts");
    const installer = await app.request("/ship/install-v5.ts");
