...
describe("routes", () => {
  ...
  test("serves existing-project ship guidance and versioned source", async () => {
    ...
    const source = await app.request("/ship/v44.ts");
    ...
    expect(sourceBody).toContain('const CURRENT_SOURCE = "https://shibumistack.dev/ship/v44.ts"');
    ...
    expect(installerRedirect.headers.get("location")).toBe("/ship/install-v42.ts");
    const installer = await app.request("/ship/install-v42.ts");
    ...
    expect(installerBody).toContain("ship/v44.ts");
