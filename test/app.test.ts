...
describe("routes", () => {
  ...
  test("serves existing-project ship guidance and versioned source", async () => {
    ...
    expect(body).toContain('fetch("/ship/v11.ts")');
    expect(body).toContain("data-copy-code");
    expect(body).not.toContain('href="/ship/v11.ts"');
    ...
    const source = await app.request("/ship/v11.ts");
    ...
    expect(installerRedirect.headers.get("location")).toBe("/ship/install-v9.ts");
    const installer = await app.request("/ship/install-v9.ts");
