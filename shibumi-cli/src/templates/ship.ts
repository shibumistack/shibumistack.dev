...
async function requestSshTarget(configHostname?: string): Promise<string | undefined> {
  ...
  const answer = await text({
    message: "SSH server",
    placeholder: "deploy@example-vps.com",
    defaultValue: suggestion,
    validate: (value) => SSH_TARGET.test(value) ? undefined : "Use an SSH host or user@host without spaces",
  });
  if (isCancel(answer)) return undefined;
  const accepted = await confirm({
    message: `Save ${answer} locally and connect?`,
    initialValue: true,
  });
  if (isCancel(accepted) || !accepted) return undefined;
  await run(["git", "config", "--local", "shibumi.server", answer]);
...
async function ensureServer(target: string): Promise<void> {
  ...
  if (version.exitCode === 0 && versionAtLeast(version.stdout.trim(), "0.1.6")) return;
  note(
