...
app.get("/install/ship", (c) => c.redirect("/ship/install-v30.ts", 302));
app.get("/install/ship.sh", (c) => c.redirect("/ship/bootstrap-v23.sh", 302));
app.get("/ship/latest.ts", async (c) => c.body(await read("public/ship/v32.ts"), 200, {
  "Cache-Control": "no-cache",
  "Content-Disposition": 'inline; filename="ship.ts"',
  "Content-Type": "text/plain; charset=utf-8",
}));
app.get("/ship/:asset", async (c) => {
  const asset = c.req.param("asset");
  if (!/^(?:v\d+|install-v\d+)\.ts$|^bootstrap-v\d+\.sh$/.test(asset)) return c.notFound();
  const path = `public/ship/${asset}`;
  if (!await Bun.file(path).exists()) return c.notFound();
  const filename = asset.startsWith("bootstrap-") ? "shibumi-ship.sh" : asset.startsWith("install-") ? "shibumi-ship.ts" : "ship.ts";
  return c.body(await read(path), 200, {
