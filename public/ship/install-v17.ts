...
const sourceUrl = "https://shibumistack.dev/ship/v19.ts";
...
interface FileSnapshot { exists: boolean; content: Uint8Array }

async function snapshot(path: string): Promise<FileSnapshot> {
  const file = Bun.file(path);
  return await file.exists() ? { exists: true, content: new Uint8Array(await file.arrayBuffer()) } : { exists: false, content: new Uint8Array() };
}

async function restore(path: string, previous: FileSnapshot): Promise<void> {
  if (previous.exists) await Bun.write(path, previous.content);
...
const previousPackage = await snapshot(packagePath);
const previousSource = await snapshot(sourcePath);
const hadNodeModules = await Bun.file(join(root, "node_modules")).exists();
const lockPaths = [join(root, "bun.lock"), join(root, "bun.lockb")];
const previousLocks = await Promise.all(lockPaths.map(snapshot));
...
if (await existingFile.exists()) {
  ...
  if (existing !== source && !knownSourceHashes.has(hash(existing))) {
    fail("scripts/ship.ts contains owned changes.\nNext: review and merge https://shibumistack.dev/ship/v19.ts manually.");
  }
}
...
if (existingInstall) {
  process.stdout.write("Updated scripts/ship.ts and added bun run ship:update. Review and commit the changes.\n");
} else {
  ...
  if (setup.exitCode !== 0) {
    await restore(packagePath, previousPackage);
    await restore(sourcePath, previousSource);
    await Promise.all(lockPaths.map((path, index) => restore(path, previousLocks[index])));
    if (!hadNodeModules) await rm(join(root, "node_modules"), { recursive: true, force: true });
    fail("ship setup did not complete. Installer changes were rolled back.\nNext: follow the setup error above, then rerun installer.");
  }
}
