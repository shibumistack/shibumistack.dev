...
async function offerSetupCommit(): Promise<"none" | "committed" | "declined"> {
  ...
  log.info(`Shibumi setup files\n${changed.map((file) => `  ${file}`).join("\n")}`);
  const accepted = await confirm({ message: "Commit Shibumi deployment setup now?", initialValue: true });
...
async function remoteSetup(target: string, force: boolean): Promise<ClientConfig> {
  ...
  const downloaded = await ssh(target, [
    SERVER_CLI, "client-config", appId, "--server-hostname", serverHostname,
  ], { allowFailure: true });
