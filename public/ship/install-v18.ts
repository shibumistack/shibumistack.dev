...
const sourceUrl = "https://shibumistack.dev/ship/v20.ts";
...
if (await existingFile.exists()) {
  ...
  if (existing !== source && !knownSourceHashes.has(hash(existing))) {
    fail("scripts/ship.ts contains owned changes.\nNext: review and merge https://shibumistack.dev/ship/v20.ts manually.");
  }
}
...
if (existingInstall) {
  ...
  const setup = await command([process.execPath, sourcePath, "--setup", "--yes", ...process.argv.slice(2)], true);
  if (setup.exitCode !== 0) {
    fail("ship setup is incomplete. Setup files were kept so setup can resume.\nNext: complete the action above, then run bun run ship:setup.");
  }
}
