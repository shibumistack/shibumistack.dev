...
- Static: optional build step, verified relative output dir (`dist`, `public`, `build`, `out`, or custom), any framework. Validation: path inside project root, exists after build, non-empty, `index.html` present, symlinks contained. File routing plus `404.html`; SPA fallback only on explicit choice. Image contains only verified output plus a pinned static server; health checks `/`. v1. Build step is a package-script name, never a free shell string. New projects only; existing framework projects onboard via `/install/ship` (resolved question 3).
...
- Blog, SPA, AI, framework-specific templates. later
...
4. Static template
   - Prompts and flags per contract; validation incl. symlink containment; pinned static server; `404.html`; opt-in SPA fallback; published exact static script table; agents.md variant.
   - Accept: fixtures for dist/public/build/out plus a valid custom dir; rejections for absolute path, escape, empty dir, missing index.html, failed build script, escaping symlink; positive file-routing and URL-traversal tests; contained symlink passes; SPA fallback only when chosen.
